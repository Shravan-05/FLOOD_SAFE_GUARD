...
export class MemStorage implements IStorage {
  ...
  async getLatestFloodRiskByUserId(
    userId: number
  ...
  async getUnreadAlertsByUserId(userId: number): Promise<Alert[]> {
    return (await this.getAlertsByUserId(userId)).filter(
      (alert) => !alert.isRead
    );
  }
  ...
  async getRoadsByArea(
    ...
    return Array.from(this.roads.values()).filter((road) => {
      ...
      return (
        (startLatDiff < radius / 111 && startLongDiff < radius / 111) ||
  ...
  async getRiverLevelsByArea(
    latitude: number,
    longitude: number,
    radius: number
  ): Promise<RiverLevel[]> {
    // Simple implementation - in a real app would use haversine distance calculation
    return Array.from(this.riverLevels.values()).filter((riverLevel) => {
  ...
  async createRiverLevel(
    insertRiverLevel: InsertRiverLevel
  ): Promise<RiverLevel> {
  ...
  async updateRiverLevel(
    id: number,
    level: number
  ): Promise<RiverLevel | undefined> {
...
export class PostgresStorage implements IStorage {
  ...
  async getRoadsByArea(
    ...
    return allRoads.filter((road) => {
      ...
      return (
        (startLatDiff < latRange && startLongDiff < longRange) ||
